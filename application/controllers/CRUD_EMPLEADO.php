...
class CRUD_EMPLEADO extends CI_Controller {
    ...
    public function AddEmpleado()
    {
        ...
        $mantenerDatos = ["rut" => $rut1, "nombre" => $nombre1, "paterno"=> $paterno1, "materno" => $materno1, "pass1" =>$pass11, "pass2" => $pass22, "correo"=> $correo1, "rol"=> $rol1];
        ...
        $error7=$this->validate_form_empleado->ValidateLargeCorreo($correo1);
        ...
        $mensaje =["1"=>$error1,"2"=>$error2,"3"=>$error3,"4"=>$error4,"5"=>$error5,"6"=>$error6,"7"=>$error7,"8"=>$error8,"9"=>$error9,"10"=>$error10];

        if(empty($mensaje['1']) && empty($mensaje['2']) && empty($mensaje['3']) && empty($mensaje['4']) && empty($mensaje['5']) && empty($mensaje['6']) && empty($mensaje['7']) && empty($mensaje['8']) && empty($mensaje['9']) && empty($mensaje['10']))
        {
            if($pass11 == $pass22)
            {
